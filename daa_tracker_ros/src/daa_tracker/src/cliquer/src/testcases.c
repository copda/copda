...
int small_max_cliques[][N] = { { 4, 0, 2, 4, 7 }, { 0 } };
int small_3_sized_cliques[][N] = { { 4, 0, 2, 4, 7 }, { 3, 2, 4, 7 }, { 3, 0, 4, 7 }, { 3, 0, 2, 7 }, { 3, 0, 2, 4 },
                                   { 3, 0, 2, 5 },    { 3, 2, 4, 6 }, { 3, 1, 4, 6 }, { 0 } };

int large_max_cliques[][N] = { { 9, 6, 29, 152, 284, 378, 388, 409, 561, 594 },
...
int large_w_max_cliques[][N] = { { 8, 5, 167, 365, 368, 446, 479, 540, 574 }, /* w=66 */
                                 { 7, 137, 148, 208, 219, 453, 460, 542 },    /* w=66 */
                                 { 0 } };
int large_w_exact_62_cliques[][N] = { { 7, 5, 158, 188, 297, 446, 479, 503 },        /* w=62 */
...
void test_single_clique(graph_t* g, int min_size, int max_size, boolean maximal, int (*sets)[N]);
void test_single_w_clique(graph_t* g, int min_size, int max_size, boolean maximal, int (*sets)[N]);
void test_all_cliques(graph_t* g, int min_size, int max_size, boolean maximal, int (*sets)[N]);
void test_all_w_cliques(graph_t* g, int min_size, int max_size, boolean maximal, int (*sets)[N]);
...
void test_reentrance(graph_t* g1, int min1, int max1, boolean maximal1, int (*sets1)[N], graph_t* g2, int min2,
                     int max2, boolean maximal2, int (*sets2)[N]);
...
int main(int argc, char* argv[])
{
  ...
  printf("\n");
  printf("Running testcases:  ELEMENTSIZE=%d, sizeof(setelement)=%d  ", ELEMENTSIZE, (int)sizeof(setelement));
  ...
  test_reentrance(wlarge, 60, 64, TRUE, large_w_60_64_maximal_cliques, small, 0, 0, FALSE, small_max_cliques);
...
void test_user_function(graph_t* g, int min_size, int (*sets)[N])
{
  ...
  n = clique_unweighted_find_all(g, min_size, 0, FALSE, clique_default_options);
...
boolean reentrance_user_function(set_t set, graph_t* graph, clique_options* opts)
{
  ...
  n = clique_find_all(sopt->g, sopt->min, sopt->max, sopt->maximal, &localopts);
