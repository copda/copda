...
static boolean sub_unweighted_single(int* table, int size, int min_size, graph_t* g);
static int sub_unweighted_all(int* table, int size, int min_size, int max_size, boolean maximal, graph_t* g,
                              clique_options* opts);
static int sub_weighted_all(int* table, int size, int weight, int current_weight, int prune_low, int prune_high,
                            int min_weight, int max_weight, boolean maximal, graph_t* g, clique_options* opts);
 ...
 */
static int unweighted_clique_search_single(int* table, int min_size, graph_t* g, clique_options* opts)
{
  struct tms tms;
  ...
  for (i = 1; i < g->n; i++)
  {
    ...
    if (opts && opts->time_function)
    {
      ...
      if (!opts->time_function(
              entrance_level, i + 1, g->n, clique_size[v] * weight_multiplier,
              (double)(tms.tms_utime - cputimer.tms_utime) / clocks_per_sec,
              timeval.tv_sec - realtimer.tv_sec + (double)(timeval.tv_usec - realtimer.tv_usec) / 1000000, opts))
      {
...
static boolean sub_unweighted_single(int* table, int size, int min_size, graph_t* g)
{
  ...
  for (i = size - 1; i >= 0; i--)
  {
    ...
    if (sub_unweighted_single(newtable, p1 - newtable, min_size - 1, g))
    {
      /* Clique found. */
      SET_ADD_ELEMENT(current_clique, v);
      temp_list[temp_count++] = newtable;
 ...
 */
static int unweighted_clique_search_all(int* table, int start, int min_size, int max_size, boolean maximal, graph_t* g,
                                        clique_options* opts)
{
  struct timeval timeval;
  struct tms tms;
  ...
  for (i = start; i < g->n; i++)
  {
    ...
    SET_ADD_ELEMENT(current_clique, v);
    j = sub_unweighted_all(newtable, newsize, min_size - 1, max_size - 1, maximal, g, opts);
                              ...
                              clique_options* opts)
{
  ...
  for (i = size - 1; i >= 0; i--)
  {
    ...
    SET_ADD_ELEMENT(current_clique, v);
    n = sub_unweighted_all(newtable, p1 - newtable, min_size - 1, max_size - 1, maximal, g, opts);
    SET_DEL_ELEMENT(current_clique, v);
    if (n < 0)
    {
      /* Abort. */
 ...
 * Note: Does NOT use opts->user_function of opts->clique_list.
 */
static int weighted_clique_search_single(int* table, int min_weight, int max_weight, graph_t* g, clique_options* opts)
{
  struct timeval timeval;
  struct tms tms;
  int i, j;
  ...
  for (i = 1; i < g->n; i++)
  {
    ...
    SET_ADD_ELEMENT(current_clique, v);
    search_weight =
        sub_weighted_all(newtable, newsize, newweight, g->weights[v], search_weight,
                         clique_size[table[i - 1]] + g->weights[v], min_w, max_weight, FALSE, g, &localopts);
    SET_DEL_ELEMENT(current_clique, v);
    if (search_weight < 0)
    {
      break;
 ...
 * in graph, if user/time_function aborts).
 */
static int weighted_clique_search_all(int* table, int start, int min_weight, int max_weight, boolean maximal,
                                      graph_t* g, clique_options* opts)
{
  ...
  for (i = start; i < g->n; i++)
  {
    ...
    SET_ADD_ELEMENT(current_clique, v);
    j = sub_weighted_all(newtable, newsize, newweight, g->weights[v], min_weight - 1, INT_MAX, min_weight, max_weight,
                         maximal, g, opts);
    SET_DEL_ELEMENT(current_clique, v);
                            ...
                            int min_weight, int max_weight, boolean maximal, graph_t* g, clique_options* opts)
{
  ...
  {
    if ((current_weight <= max_weight) && ((!maximal) || is_maximal(current_clique, g)))
    {
      /* We've found one.  Store it. */
      if (!store_clique(current_clique, g, opts))
      {
  ...
  for (i = size - 1; i >= 0; i--)
  {
    ...
    SET_ADD_ELEMENT(current_clique, v);
    prune_low = sub_weighted_all(newtable, p1 - newtable, newweight, current_weight + w, prune_low, prune_high,
    ...
    if ((max_size > 0) && (set_size(current_clique) > max_size))
    {
      ...
      for (i = 0; i < g->n - 1; i++)
        if (clique_size[table[i]] >= min_size)
          break;
      if (unweighted_clique_search_all(table, i, min_size, max_size, maximal, g, &localopts))
      {
        set_free(current_clique);
        current_clique = s;
      }
...
int clique_unweighted_find_all(graph_t* g, int min_size, int max_size, boolean maximal, clique_options* opts)
{
  ...
  for (i = 0; i < g->n - 1; i++)
    if (clique_size[table[i]] >= min_size)
      break;
  count = unweighted_clique_search_all(table, i, min_size, max_size, maximal, g, opts);
...
set_t clique_find_single(graph_t* g, int min_weight, int max_weight, boolean maximal, clique_options* opts)
{
  ...
  if (!graph_weighted(g))
  {
    ...
    entrance_level--;
    s = clique_unweighted_find_single(g, min_weight, max_weight, maximal, opts);
    ENTRANCE_RESTORE();
    return s;
  }
  ...
  if (weighted_clique_search_single(table, min_weight, max_weight, g, opts) == 0)
  {
    /* Requested clique has not been found. */
  ...
  if (maximal && (min_weight > 0))
  {
    ...
    if (graph_subgraph_weight(g, best_clique) > max_weight)
    {
      ...
      for (i = 0; i < g->n - 1; i++)
        if ((clique_size[table[i]] >= min_weight) || (clique_size[table[i]] == 0))
          break;
      if (!weighted_clique_search_all(table, i, min_weight, max_weight, maximal, g, &localopts))
      {
        set_free(best_clique);
        best_clique = NULL;
      }
 ...
 *       weights are the same.
 */
int clique_find_all(graph_t* g, int min_weight, int max_weight, boolean maximal, clique_options* opts)
{
  int i, n;
  int* table;
  ...
  if (!graph_weighted(g))
  {
    ...
    weight_multiplier = g->weights[0];
    entrance_level--;
    i = clique_unweighted_find_all(g, min_weight, max_weight, maximal, opts);
    ENTRANCE_RESTORE();
    return i;
  }
  ...
  /* Second phase */
  n = weighted_clique_search_all(table, i, min_weight, max_weight, maximal, g, opts);
...
boolean clique_print_time(int level, int i, int n, int max, double cputime, double realtime, clique_options* opts)
{
  ...
  if (ABS(prev_time - realtime) > 0.1 || i == n || i < prev_i || max != prev_max || level != prev_level)
  {
    for (j = 1; j < level; j++)
      fprintf(fp, "  ");
