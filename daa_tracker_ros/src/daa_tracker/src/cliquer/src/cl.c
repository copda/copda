...
void read_options(int argc, char** argv)
{
  ...
  while (TRUE)
  {
    ...
    static struct option long_options[] = {
      { "all", no_argument, NULL, 'a' },        { "single", no_argument, NULL, 's' },
      { "weight", no_argument, NULL, 'w' },     { "min", required_argument, NULL, 'm' },
      { "max", required_argument, NULL, 'M' },  { "maximal", no_argument, NULL, 'x' },
      { "unweighted", no_argument, NULL, 'u' }, { "reorder", required_argument, NULL, 'r' },
      { "from-0", no_argument, NULL, '0' },     { "quiet", no_argument, NULL, 'q' },
      { "help", no_argument, NULL, 'h' },       { 0, 0, 0, 0 }
    };

    c = getopt_long(argc, argv, "aswm:M:xur:1qh", long_options, &option_index);
...
void print_search(graph_t* g)
{
  ...
  fprintf(stderr, "clique%s with %s ", find_all ? "s" : "", unweighted ? "size" : "weight");
  if (max_weight == 0)
  {
...
boolean record_clique_func(set_t s, graph_t* g, clique_options* opts)
{
  ...
  {
    clique_list = realloc(clique_list, (clique_list_size + 512) * sizeof(set_t));
