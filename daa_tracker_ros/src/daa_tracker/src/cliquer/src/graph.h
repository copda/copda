...
#define GRAPH_IS_EDGE(g, i, j) (((i) < ((g)->n)) ? SET_CONTAINS((g)->edges[(i)], (j)) : FALSE)
...
extern boolean graph_write_dimacs_ascii_file(graph_t* g, char* comment, char* file);
extern boolean graph_write_dimacs_binary(graph_t* g, char* comment, FILE* fp);
extern boolean graph_write_dimacs_binary_file(graph_t* g, char* comment, char* file);
