...
static int set_bit_count[256] = { 0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4, 1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4, 3,
...
#define SET_ELEMENT_BIT_COUNT_16(a) (set_bit_count[(a) >> 8] + set_bit_count[(a)&0xFF])
#define SET_ELEMENT_BIT_COUNT_32(a)                                                                                    \
  (set_bit_count[(a) >> 24] + set_bit_count[((a) >> 16) & 0xFF] + set_bit_count[((a) >> 8) & 0xFF] +                   \
   set_bit_count[(a)&0xFF])
#define SET_ELEMENT_BIT_COUNT_64(a)                                                                                    \
  (set_bit_count[(a) >> 56] + set_bit_count[((a) >> 48) & 0xFF] + set_bit_count[((a) >> 40) & 0xFF] +                  \
   set_bit_count[((a) >> 32) & 0xFF] + set_bit_count[((a) >> 24) & 0xFF] + set_bit_count[((a) >> 16) & 0xFF] +         \
...
#define SET_CONTAINS_FAST(s, a) (SET_ELEMENT_CONTAINS((s)[(a) / ELEMENTSIZE], (a) % ELEMENTSIZE))
...
UNUSED_FUNCTION INLINE static set_t set_resize(set_t s, int size)
{
  ...
  if (n > SET_ARRAY_LENGTH(s))
    memset(s + SET_ARRAY_LENGTH(s), 0, (n - SET_ARRAY_LENGTH(s)) * sizeof(setelement));
  if (size < SET_MAX_SIZE(s))
    s[(size - 1) / ELEMENTSIZE] &= (FULL_ELEMENT >> (ELEMENTSIZE - size % ELEMENTSIZE));
  s[-1] = size;
...
UNUSED_FUNCTION INLINE static set_t set_copy(set_t dest, set_t src)
{
  ...
  memset(dest + SET_ARRAY_LENGTH(src), 0, ((SET_ARRAY_LENGTH(dest) - SET_ARRAY_LENGTH(src)) * sizeof(setelement)));
  return dest;
}
 ...
 */
UNUSED_FUNCTION INLINE static set_t set_intersection(set_t res, set_t a, set_t b)
{
  int i, max;
